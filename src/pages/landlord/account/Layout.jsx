...
import { FaMoneyCheckDollar, FaUserGear, FaDoorOpen } from "react-icons/fa6";
...
const Layout = () => {
  ...
  const LogoutModal = () => (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 px-4 h-dvh z-50">
      <div className="bg-white rounded-2xl w-full max-w-md p-6 space-y-4">
        ...
        <div className="flex justify-end gap-3 pt-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-red-600 text-white text-sm hover:bg-red-700 transition disabled:opacity-70 flex items-center gap-2"
          >
            {logoutLoading ? (
              <>
  ...
  return (
    <>
      ...
      {/* Single modal instance at top level */}
          ...
          <header className="py-3 md:py-4 bg-white/70 backdrop-blur-xl border-b border-white/30 flex items-center justify-between px-6 shadow-sm">
            <div>
              <h1 className="text-md md:text-lg font-semibold text-gray-900">
                <span className="bg-black text-xs md:text-sm text-white px-2 py-1 rounded-full">Landowner</span>
              </h1>
            ...
            <div className="relative gap-3 md:gap-5 flex items-center text-sm">
              <Link to="/auth/landowner/notifications" className="relative text-2xl">
              ...
              {/* Mobile dropdown */}
              {open && (
                <div className="absolute right-0 top-12 w-32 bg-white border rounded-lg shadow-md z-40">
                  <button
                    onClick={() => {
                      setOpen(false);
                      setShowModal(true);
                    }}
                    className="w-full text-left p-4 rounded-xl hover:bg-neutral-50 text-sm"
                  >
                    Logout
                  </button>
                </div>
          ...
          <div className="flex justify-around items-center gap-1 h-16 px-1">
            {navItems.map(item => {
              const isActive =
                ...
                (item.to === "/auth/landowner" && location.pathname === "/auth/landowner");
